...
// PostgresWriter writes CDC changes to PostgreSQL.
...
// NewPostgresWriter creates a new PostgreSQL writer.
...
func (w *PostgresWriter) IsHealthy(ctx context.Context) bool {
	return w.pool.Ping(ctx) == nil
}

// ApplyChanges applies a batch of changes within a transaction.
func (w *PostgresWriter) ApplyChanges(ctx context.Context, changes []*proto.ChangeRequest, idempotent bool) (uint64, error) {
...
func (w *PostgresWriter) upsert(ctx context.Context, tx pgx.Tx, change *proto.ChangeRequest) (int64, error) {
	table := fmt.Sprintf("%s.%s", change.SchemaName, change.TableName)
...
func (w *PostgresWriter) insert(ctx context.Context, tx pgx.Tx, change *proto.ChangeRequest) (int64, error) {
	table := fmt.Sprintf("%s.%s", change.SchemaName, change.TableName)
...
func (w *PostgresWriter) delete(ctx context.Context, tx pgx.Tx, change *proto.ChangeRequest) (int64, error) {
	table := fmt.Sprintf("%s.%s", change.SchemaName, change.TableName)
...
func (w *PostgresWriter) getPrimaryKeyColumns(ctx context.Context, change *proto.ChangeRequest) ([]string, error) {
	tableKey := fmt.Sprintf("%s.%s", change.SchemaName, change.TableName)

	// Check cache first
	w.cacheMu.RLock()
	if pks, ok := w.pkCache[tableKey]; ok {
		w.cacheMu.RUnlock()
		return pks, nil
	}
	w.cacheMu.RUnlock()
	...
	if len(pkFromMetadata) > 0 {
		w.cacheMu.Lock()
		w.pkCache[tableKey] = pkFromMetadata
		w.cacheMu.Unlock()
		return pkFromMetadata, nil
	}

	// Fall back to querying PostgreSQL
	query := `
		SELECT a.attname
		FROM pg_index i
	...
	rows, err := w.pool.Query(ctx, query, tableKey)
	if err != nil {
